...
- **End-to-end encryption**: keys are derived from the password on each device (PBKDF2 + X25519 + HKDF + AES-256-GCM). The server only ever sees ciphertext — messages *and* media.
...
- **Baked-in backend**: the app always connects to your server (`https://your.domain.example`). The address never appears in the UI and users can't change it — you simply rebuild with `-PserverUrl=...` if you ever move servers.
...
- **Modern grayscale UI**: Jetpack Compose + Material 3 in a custom grayscale palette (Void Black / Carbon / Steel / Industrial Gray / Tech Silver).
...
android/                Android app (Kotlin + Jetpack Compose)
  app/src/main/java/app/textapp/
    crypto/             PBKDF2/X25519/HKDF/AES-GCM + Android Keystore wrapper
...
Then build the app. The backend address is baked in (default `https://your.domain.example`,
